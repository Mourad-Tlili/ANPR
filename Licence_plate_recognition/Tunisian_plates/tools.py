...
def histogram_of_pixel_projection(img):
    ...
    for i in range(len(rect_h) - 1):
        ...
        if (diff1 > 5) and (z[rect_h[i]] == 1):
            ...
            Cropped=img[int(rectv[0]):int(rectv[1]), int(rect_h[i]):int(rect_h[i + 1])]
            #Cropped=img[int(rectv[0]):int(rect_h[len(rect_h) - 1]), int(rect_h[len(rect_h) - 1]):int(rect_h[0])]
            t=cv2.resize(Cropped,(200,50))
            cv2.imwrite("final.jpg", t)
            #text44 = pytesseract.image_to_string(t, config='-l fra --oem 1 --psm 4') 
            #print("resultat"+str(i))
            #print(text44)
            
            cv2.rectangle(img, (rect_h[i], rectv[0]), (rect_h[i + 1], rectv[1]), (0, 255, 0), 1)
            #(top,left)(button,right)
            #cv2.rectangle(img, (rect_h[len(rect_h) - 1], rectv[0]), (rect_h[0], rect_h[len(rect_h) - 1]), (0, 255, 0), 1)
