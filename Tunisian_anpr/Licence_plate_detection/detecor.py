...
def drawPred(classId, conf, left, top, right, bottom):
   ...
   # print(image.size)
	...
	if len(words)==1:
		...
		pm1=pytesseract.image_to_string(Cropped, config='-l ara --oem 1 --psm 3')
		if k.find("و") != -1 and k.find("و") != len(k)-1 or k.find("ن") != -1 and k.find("ن") != len(k)-1:
			...
			if k.find("L") == -1:
				...
				results1=arabicocr.arabic_ocr(image,out_image)
				print(results1)
	...
	if len(words)==2:
		...
		k1 = "".join(words[0])
		k2 = "".join(words[1])
		pm2=pytesseract.image_to_string(Cropped, config='-l ara --oem 1 --psm 3')
		#print("matricule =")
		#print(k1+"TN"+k2[0:4])
		
		 
		if k2.find("ن") == -1 and pm2.find('ن') !=-1 and pm2.find('ن') !=len(pm2)-1 or pm2.find('ت') !=-1 and pm2.find('ت') !=len(pm2)-1:
			print("matricule2=")
			k1=k1.replace('&','6').replace('ن','')
	...
	if len(words)==3:
	    ...
	    k2 = "".join(words[2])
	    k3 = "".join(words[1])
	    if k3.find("ت") !=-1 and k3.find("و") != -1 or k3.find("ن") != -1:
	       print("matricule3 =")
	       print(k1+"TN"+k2)		
...
for i, box in enumerate(boxes):
	...
	cv.putText(image, classes[i] , (int(box[0]), int(box[1] - 5)), cv.FONT_HERSHEY_SIMPLEX, 0.8, color, 2,
				lineType=cv.LINE_AA)
	if classes[i] =="car":
		cv.imwrite("couleur"+str(i)+".jpg", Cropped)
...
with mysql.connector.connect(**connection_params) as db :
	with db.cursor() as c:
		c.execute("insert into utilisateur (nom, score, actif,date) \
				   values ('"+matricule[0]+"', '"+objcouleur[0]+"','"+objNature[0]+"', '"+str(datetime.datetime.now())+"')")
		db.commit()
matricule.clear()
objcouleur.clear()
objNature.clear() 
